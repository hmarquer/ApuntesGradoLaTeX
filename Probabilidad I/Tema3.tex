...
\begin{ejem}
	\begin{enumerate}
		...
		\item Sea $X\sim\normal{0, 1}$, entonces $\ds f_X(x)=\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}$.
		      ...
		      \[V(X) = E(X^2) - E(X)^2 = \int_{-\infty}^{\infty} x^2 \cdot \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} \odif{x} - 0 = 1\] %TODO: hacer desarrollo por partes
		\item Sea $X\sim\normal{\mu, \sigma^2}$, entonces $\ds f_X(x)=\frac{1}{\sqrt{2\pi}\sigma} e^{-\frac{(x-\mu)^2}{2\sigma^2}}$.
		      ...
		      \[V(X) = E(X^2) - E(X)^2 = \int_{-\infty}^{\infty} x^2 \cdot \frac{1}{\sqrt{2\pi}\sigma} e^{-\frac{(x-\mu)^2}{2\sigma^2}} \odif{x} - \mu^2 = \sigma^2\] %TODO: hacer desarrollo usando la función Gamma
	\end{enumerate}
\end{ejem}
\fecha{21/03/2024}
\subsubsection{Calculando con la normal $\left(\normal{\mu, \sigma^2}\right)$}
...
Caso particular de $X\sim \normal{0,1}$ %TODO: Hacer desarrollo
\[E(X^k) = \begin{cases}
...
\textbf{¿Qué hago si me dan $P(X \leq a)$ y me piden $a$?}
...
\textbf{Nota sobre independencia:}
